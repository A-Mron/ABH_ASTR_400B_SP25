...
# -*- coding: utf-8 -*-
"""
Created on Sat May 10 15:48:58 2025
...
def ScaleRadius(a):
    ...
    r_arrMW = np.linspace(5, 40, 100)
    r_arrM31 = np.linspace(5, 40, 100)
    
    # calculate mass profiles
    MW_halo = mwProf.massEnclosed(1, r_arrMW)
    M31_halo = m31Prof.massEnclosed(1, r_arrM31)
    #Concatenate mass profile arrays and radius arrays
    mcombo = MW_halo + M31_halo
    ...
    fig, ax = plt.subplots()
    # lines
    ax.plot(r_arr, Hernquist(r_arr, a, M_halo_tot)/1e12, 
            c='r', label='Analytic Halo, a={86} kpc')
    ax.plot(r_arr, mcombo, c='b', linestyle='-.', label='Halo')
    ax.set(xlabel='r [kpc]', ylabel='$M_{enc}$ $[M_\\odot]$', 
          title='Merged Halo Mass Profile')
    ax.legend()
    #.format(a)
ScaleRadius(86)    
# Merged scla eradius: 86
...
def M33COM():
    ...
    '''
    COM = CenterOfMass('M33_650.txt',2) #Initalizw class
    
    M33pos = COM.COM_P(0.1,4) #calc COM position
    print(M33pos)
...
#Need to use disk particles since the halo particles are so shredded (far apart)
#that the code is putting out errors
...
plt.figure()
plt.plot(r,V_pm,'r')
plt.scatter(pos.value,vel.value,color = 'g',linewidth = 3, label='M33')
...
#Hernquist 
plt.figure()
R_Hernquist,V_Hernquist = Hernquist_approx(88)
plt.plot(R_Hernquist,V_Hernquist,color='k')
plt.scatter(pos.value,vel.value,color = 'g', linewidth = 3, label = 'M33')
plt.xlabel('Radius [Kpc]')
...
plt.xlabel('Point Mass $V_{esc}$ [Km/s]')
plt.ylabel('Hernquist $V_{esc}$ [Km/S]')
